...
public class AIUtil {
	...
	public static void answerAIMsg(String content, JSONObject userMsg,Session session) {
		...
		if(session.getAiCount()>=Integer.valueOf(count)) {
			...
			answer = parseResult(HttpUtil.postForObject(TULING_OPENAPI_URL, getAIParams(content,userMsg.getString("FromUserName"))));
		}
		WXCenterUtil.sendCustMsg(WechatMsgFactory.createCustMsg(getCustMsgParams(answer), userMsg.getString("FromUserName")), userMsg.getString("ToUserName"), userMsg.getString("domain2"));
	}
	...
	private static JSONObject getCustMsgParams(String content) {
		JSONObject params = new JSONObject();
		params.put("msg_type", WXConstants.MsgConstants.WX_CUST_MSG_TYPE_TEXT);
