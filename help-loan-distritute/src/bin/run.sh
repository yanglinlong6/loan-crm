...
JAVA_OPTS_PRO="
    ...
    -XX:HeapDumpPath=/data/logs/distribute/dump/distribute.hprof
    -XX:+PrintGCDetails -XX:+PrintGCTimeStamps
    -XX:+PrintGCTimeStamps -Xloggc:/data/logs/distribute/dump/gc.log"
...
start(){
    if [ -f $pid_file ];then
        ...
        java $JAVA_OPTS_PRO -Dloader.path=$WORKDIR/lib -jar $work_path/distribute-*.jar  --spring.profiles.active=pro  >/dev/null 2>&1 &
