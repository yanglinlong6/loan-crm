 ...
 * 南京戎之竭力
 */
@Component("apiSender_10013")
public class RongZhiJieLiApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(RongZhiJieLiApi.class);

    private static final String checkUrl = "http://data.xiaoweiws.com/data/api.hit/duplicatepzzk?mobile=%s&department_id=%s";

    private static final String checkUrl2 = "http://dbz.huidaikeji.com/api/api/colliding?mobile=%s";

    private static final String sendUrl = "http://dbz.huidaikeji.com/api/api/addzq";

    private static final String code = "fxskzqtg01";

    private static final String key = "DbzTJ2txzXiaoLia";

    private static final String flag = "fxsk";
    
    private static final String depId="469";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            log.error("【南京戎之竭力】分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京戎之竭力】分发未知异常："+e.getMessage()));
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select) throws Exception {
        ...
        String url = String.format(checkUrl,MD5Util.getMd5String(po.getMobile()),depId);
        String checkResult = HttpUtil.getForObject(url);
        log.info("【南京戎之竭力】撞库2结果：{}",checkResult);
        //{ "code": "200", "message": "请求成功" }
        int resultCode = JSONUtil.toJSON(checkResult).getIntValue("code");
        if(200 != resultCode){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【南京戎之竭力】分发重复："+checkResult));
        ...
        if(500 == resultJson.getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【南京戎之竭力】分发成功："+result));
            return new SendResult(true,result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京戎之竭力】分发失败："+result ));
...
//        RongZhiJieLiApi api = new RongZhiJieLiApi();
