...
/**
 * 上海努作金融信息服务有限公司：上海，杭州：apiSender_20008
 */
@Component("apiSender_20008")
public class WangShangApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(WangShangApi.class);
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            if(org.apache.commons.lang3.StringUtils.isNotBlank(po.getCity()) && "苏州市".equals(po.getCity())){
                return sendSuZhou(po,select);
            }
            return send2(po,select);
        }catch (Exception e){
            log.error(e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【网商】分发未知异常："+e.getMessage()));
            return new SendResult(false,"【网商】发送异常："+e.getMessage());
        }
    ...
    private SendResult send2(UserAptitudePO po, UserDTO select) throws Exception {
        ...
        log.info("【网商】验证手机号码结果：{}",checkResult);
        //{"code":200,"message":"请求成功"}
        if(200 != JSONUtil.toJSON(checkResult).getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【网商】分发重复："+checkResult));
            return new SendResult(false,"【网商】验证手机号码:重复："+checkResult);
        }
        JSONObject data = new JSONObject();
        if(StringUtils.isNotBlank(po.getChannel()) && po.getChannel().contains("ttt")){//头条
            data.put("code","xdhctttg01");
            data.put("flag","tt");
        }else{//朋友圈
            data.put("code","xdhcpyqtg01");
            data.put("flag","pyq");
        }
        ...
        log.info("【网商】分发结果：{}",result);
        JSONObject resultJSON = JSONUtil.toJSON(result);
        if(500 == resultJSON.getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【网商】分发成功"+result));
            return new SendResult(true,"【网商】:"+result);
        }
        String msg = resultJSON.getString("msg");
        if("电话号码重复".equals(msg)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【网商】分发重复"+result));
            return new SendResult(false,"【网商】"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【网商】分发失败："+result));
        return new SendResult(false,"【网商】分发失败"+result);
    }
    ...
    private SendResult sendSuZhou(UserAptitudePO po, UserDTO select) throws Exception {
        ...
        log.info("【网商-苏州】验证手机号码结果：{}",checkResult);
        //{"code":200,"message":"请求成功"}
        if(200 != JSONUtil.toJSON(checkResult).getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【网商-苏州】分发重复："+checkResult));
            return new SendResult(false,"【网商-苏州】验证手机号码:重复："+checkResult);
        }
        JSONObject data = new JSONObject();
        if(StringUtils.isNotBlank(po.getChannel()) && po.getChannel().contains("ttt")){//头条
            data.put("code","xdhctttg01");
        ...
        JSONObject resultJSON = JSONUtil.toJSON(result);
        if(500 == resultJSON.getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【网商-苏州】分发成功"+result));
            return new SendResult(true,"【网商-苏州】:"+result);
        }
        String msg = resultJSON.getString("msg");
        if("电话号码重复".equals(msg)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【网商-苏州】分发重复"+result));
            return new SendResult(false,"【网商-苏州】"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【网商-苏州】分发失败："+result));
        return new SendResult(false,"【网商-苏州】分发失败"+result);
    }
...
//        WangShangApi api = new WangShangApi();
