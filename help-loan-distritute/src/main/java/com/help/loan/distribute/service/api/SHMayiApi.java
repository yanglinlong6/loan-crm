 ...
 * 上海蚂蚁速代信息科技有限公司
 */
@Component("apiSender_20124")
public class SHMayiApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(SHMayiApi.class);

    private static final String checkUrl = "https://api.mayizhudai.com/sem_oo/check_loan.html?MobilePhone=%s";

    private static final String sendUrl = "https://api.mayizhudai.com/sem/loan_do_sh.html";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[上海蚂蚁]推送异常：{},{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[上海蚂蚁]推送异常:"+e.getMessage()));
            return new SendResult(false,"[上海蚂蚁]推送异常："+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("[上海蚂蚁]撞库结果:{}",result);
        JSONObject resultJSON =JSONUtil.toJSON(result);
        if(0 == resultJSON.getIntValue("Ret") && 101 == resultJSON.getIntValue("Code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[上海蚂蚁]推送重复:"+result));
            return new SendResult(false,"[上海蚂蚁]推送重复："+result);
        }
        isHaveAptitude(po);
        ...
        if(Long.valueOf(result).longValue() > 1000){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[上海蚂蚁]推送成功:"+result));
            return new SendResult(true,"[上海蚂蚁]推送成功："+result);
        }else if("3".equals(result)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[上海蚂蚁]推送重复:"+result));
            return new SendResult(false,"[上海蚂蚁]推送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[上海蚂蚁]推送失败:"+result));
        return new SendResult(false,"[上海蚂蚁]推送失败："+result);
    }
