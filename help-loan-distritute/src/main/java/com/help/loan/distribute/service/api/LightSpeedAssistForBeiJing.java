 ...
 *  北京光速助力科技有限公司 20146
 */
@Component("apiSender_20146")
public class LightSpeedAssistForBeiJing implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(LightSpeedAssistForBeiJing.class);
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            log.error("[北京光速助力科技有限公司]推送异常:{}", e.getMessage(), e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京光速助力科技有限公司]推送异常:" + e.getMessage()));
            return new SendResult(false, "[北京光速助力科技有限公司]推送异常:" + e.getMessage());
        }
    }
...
//        log.info("[北京光速助力科技有限公司]撞库结果:{}", checkResult);
//        if (JSONUtil.toJSON(checkResult).getIntValue("Code") != 100) {
//            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[北京光速助力科技有限公司]推送重复:" + checkResult));
//            return new SendResult(false, "[北京光速助力科技有限公司]撞库重复:" + checkResult);
//        }
        isHaveAptitude(po);
        ...
        data.put("weili", 0);
        data.put("customerId", po.getId());
        System.out.println(data.toJSONString());

        String url = "https://api.zhongyaoqg.com/sem_yf/loan_into.html";
//        String result = HttpUtil.postFormForObject(url, data);
        String result = HttpUtil.postForJSON(url, data);
        log.info("[北京光速助力科技有限公司]推送结果:{}", result);
        if (JSONUtil.toJSON(result).getIntValue("code") == 200) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 1, "[北京光速助力科技有限公司]推送成功:" + result));
            return new SendResult(true, "[北京光速助力科技有限公司]推送成功：" + result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京光速助力科技有限公司]推送失败:" + result));
        return new SendResult(false, "[北京光速助力科技有限公司]推送失败：" + result);
    }
...
//        po.setUpdateDate(new Date());
        ...
        po.setUpdateDate(new Date());
        ApiSender api = new LightSpeedAssistForBeiJing();
