...
/*
 * 	北京鑫晟银通信息咨询有限公司 20151
 */
@Component("apiSender_20151")
public class XinShengYinTongInformationForBeijingApi implements ApiSender {
    private static final Logger LOG = LoggerFactory.getLogger(XinShengYinTongInformationForBeijingApi.class);
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            LOG.error("[北京鑫晟银通信息咨询有限公司]推送异常：{}", e.getMessage(), e);
            if (!JudgeUtil.startWith(po.getChannel(), "360")) {
                dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京鑫晟银通信息咨询有限公司]推送异常:" + e.getMessage()));
            }
            return new SendResult(false, "[北京鑫晟银通信息咨询有限公司]推送异常：" + e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select) {
        ...
        LOG.info("[北京鑫晟银通信息咨询有限公司]推送结果：{}", result);
        int code = json.getIntValue("code");
        if (200 == code) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 1, "[北京鑫晟银通信息咨询有限公司]推送成功：" + result));
            return new SendResult(true, "[北京鑫晟银通信息咨询有限公司]推送成功：" + result);
        } else if (601 == code) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[北京鑫晟银通信息咨询有限公司]推送重复：" + result));
            return new SendResult(false, "[北京鑫晟银通信息咨询有限公司]推送重复：" + result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京鑫晟银通信息咨询有限公司]推送失败：" + result));
        return new SendResult(false, "[北京鑫晟银通信息咨询有限公司]推送失败：" + result);
    }
    ...
    public static void main(String[] args) {
        ...
        ApiSender api = new XinShengYinTongInformationForBeijingApi();
