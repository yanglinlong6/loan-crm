 ...
 * 北京未来顺金信息咨询有限公司
 */
@Component("apiSender_20134")
public class BJWeilaishunjingApi implements ApiSender{

    private static final Logger LOG = LoggerFactory.getLogger(BJWeilaishunjingApi.class);

    private static final String key = "fluxZhudai*_key";

    /**租户id*/
    private static final String tenanId = "1587426119245221889";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京未来顺金]推送未知异常："+e.getMessage()));
            LOG.error("[北京未来顺金]推送异常:{}-{}",e.getMessage(),e);
            return new SendResult(false,"[北京未来顺金]推送异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京未来顺金]撞库重复："+result));
        return new SendResult(false,"[北京未来顺金]撞库重复:"+result);
    }
    private SendResult register(UserAptitudePO po, UserDTO select){
        ...
        customer.put("provinceCode",310000); // 上海市
        customer.put("cityCode",310100);//上海市
        JSONArray customers = new JSONArray();
        ...
        if(200 == code
                && 0 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[北京未来顺金]推送成功："+result));
            return new SendResult(true,"[北京未来顺金]推送成功:"+result);
        }
        if(200 == code
                && 1 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京未来顺金]推送重复："+result));
            return new SendResult(false,"[北京未来顺金]推送重复:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[享贷科技]推送失败："+result));
        return new SendResult(false,"[北京未来顺金]推送失败:"+result);
    }
...
//
//        ApiSender apiSender = new BJWeilaishunjingApi();
