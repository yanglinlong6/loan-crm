...
/*
 *  北京安久助代信息科技有限公司 20148
 */
@Component("apiSender_20148")
public class AnJiuZhuDaiInformationForBeijingApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(AnJiuZhuDaiInformationForBeijingApi.class);
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            log.error("[北京安久助代信息科技有限公司]推送异常:{}", e.getMessage(), e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京安久助代信息科技有限公司]推送异常:" + e.getMessage()));
            return new SendResult(false, "[北京安久助代信息科技有限公司]推送异常:" + e.getMessage());
        }
    }
    ...
    private SendResult sendResultForJinan(UserAptitudePO po, UserDTO select) {
        ...
        String checkResult = HttpUtil.postFormForObject("https://api.xiaodaizhongxin.com/sem_oo/check_loan.html", checkData);
        log.info("[北京安久助代信息科技有限公司]撞库结果:{}", checkResult);
        if (JSONUtil.toJSON(checkResult).getIntValue("Code") != 100) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[北京安久助代信息科技有限公司]推送重复:" + checkResult));
            return new SendResult(false, "[北京安久助代信息科技有限公司]撞库重复:" + checkResult);
        }
        isHaveAptitude(po);
        ...
        String url = "https://api.xiaodaizhongxin.com/sem/loan_do.html";
        String result = HttpUtil.postFormForObject(url, data);
        log.info("[北京安久助代信息科技有限公司]推送结果:{}", result);
        if (Long.parseLong(result) > 1000) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 1, "[北京安久助代信息科技有限公司]推送成功:" + result));
            return new SendResult(true, "[北京安久助代信息科技有限公司]推送成功：" + result);
        } else if ("3".equals(result)) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[北京安久助代信息科技有限公司]推送重复:" + result));
            return new SendResult(false, "[北京安久助代信息科技有限公司]推送重复：" + result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京安久助代信息科技有限公司]推送失败:" + result));
        return new SendResult(false, "[北京安久助代信息科技有限公司]推送失败：" + result);
    }
...
//        po.setUpdateDate(new Date());
        ...
        po.setUpdateDate(new Date());
        ApiSender api = new AnJiuZhuDaiInformationForBeijingApi();
