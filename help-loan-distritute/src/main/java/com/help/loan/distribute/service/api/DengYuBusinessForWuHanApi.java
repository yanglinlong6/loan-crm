 ...
 *  武汉市登钰商务咨询有限公司  20147
 */
@Component("apiSender_20147")
public class DengYuBusinessForWuHanApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(DengYuBusinessForWuHanApi.class);
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            log.error("[武汉市登钰商务咨询有限公司]推送异常:{}", e.getMessage(), e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[武汉市登钰商务咨询有限公司]推送异常:" + e.getMessage()));
            return new SendResult(false, "[武汉市登钰商务咨询有限公司]推送异常:" + e.getMessage());
        }
    ...
    private SendResult sendResultForJinan(UserAptitudePO po, UserDTO select) {
        ...
        String checkURL = "http://101.132.156.35/api/customer/check_mobile";
        String checkResult = HttpUtil.postForJSON(checkURL, checkData);
        log.info("[武汉市登钰商务咨询有限公司]撞库结果:{}", checkResult);
        if (JSONUtil.toJSON(checkResult).getIntValue("code") != 0) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[武汉市登钰商务咨询有限公司]推送重复:" + checkResult));
            return new SendResult(false, "[武汉市登钰商务咨询有限公司]撞库重复:" + checkResult);
        }
        ...
        data.put("passwd", "X65c2uAAJ3F");
        System.out.println(data.toJSONString());
        String url = "http://101.132.156.35/api/customer/import_data";
        String result = restTemplate.postForObject(url, data, String.class);
        log.info("[武汉市登钰商务咨询有限公司]推送结果:{}", result);
        JSONObject jsonObject = JSONUtil.toJSON(result);
        int code = jsonObject.getIntValue("code");
        if (0 == code) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 1, "[武汉市登钰商务咨询有限公司]推送成功:" + result));
            return new SendResult(true, "[武汉市登钰商务咨询有限公司]推送成功:" + result);
        } else {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[武汉市登钰商务咨询有限公司]推送失败:" + result));
        }
        return new SendResult(false, "[武汉市登钰商务咨询有限公司]推送失败:" + result);
    }
    ...
    public static void main(String[] args) {
        ...
        ApiSender api = new DengYuBusinessForWuHanApi();
