...
public class OrgBO extends OrgPO {
     ...
     * 校验是否使用昨日的标志
     *
     * @param userAptitude 用户数据
     * @return
     */
    public OrgBO checkUseLegacyFlag(UserAptitudePO userAptitude) {
        // -1 表示不使用昨日遗留
        if (-1 == this.getOrgAptitude().getUseLegacyFlag()) {
            String today = DateUtil.formatToString(new Date(), "yyyy-MM-dd");
            if (!today.equals(DateUtil.formatToString(userAptitude.getCreateDate(), "yyyy-MM-dd"))) {
                log.info(getOrgName() + "==不使用昨日遗留;城市==" + getOrgAptitude().getCity());
                this.setCheckPass(false);
            }
        }
        log.info(getOrgName() + "==使用昨日遗留;城市==" + getOrgAptitude().getCity());
        return this;
    }
