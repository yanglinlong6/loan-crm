 ...
 * 13726022584
 */
@Component("apiSender_20043")
public class BaiRongHZApi implements ApiSender{

    private static final Logger log = LoggerFactory.getLogger(BaiRongHZApi.class);
    ...
    private static String channel = "hzwq02";

    private static String token = "4df0edab176142b932169b6f7942b157";
...
//                return new SendResult(false,"【杭州正佰融】不接收[头条]客户");
//            }
            return sendResult2(po,select);
        }catch (Exception e){
            log.error("[杭州正佰融]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【杭州正佰融】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[杭州正佰融]分发异常:"+e.getMessage());
        }
    }
    ...
    private SendResult sendResult2(UserAptitudePO po, UserDTO select){
        ...
        log.info("【杭州正佰融】验证重复:{}",result);
        if(0 != JSONUtil.toJSON(result).getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【杭州正佰融】分发重复:"+result));
            return new SendResult(false,"【杭州正佰融】验证重复:"+result);
        }
        ...
        log.info("【杭州正佰融】分发结果：{}",result);
        if(0 == JSONUtil.toJSON(result).getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【杭州正佰融】分发成功:"+result));
            return new SendResult(true,"【杭州正佰融】分发成功："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【杭州正佰融】分发失败:"+result));
        return new SendResult(false,"【杭州正佰融】分发失败："+result);
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("[杭州正佰融]获取加密key结果:{}",keyReuslt);
        JSONObject json = JSONUtil.toJSON(keyReuslt);
        if(0 != json.getIntValue("code")){
            return new SendResult(false,"[杭州正佰融]获取可以失败:"+json.getString("msg"));
        }
        ...
        log.info("【杭州正佰融】分发结果：{}",msg);
        if(0==code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【杭州正佰融】http表单分发成功："+msg));
            return new SendResult(true,"【杭州正佰融】分发成功：{}"+msg);
        }else if(-3 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【杭州正佰融】分发重复："+msg));
            return new SendResult(false,"【杭州正佰融】分发重复：{}"+msg);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【杭州正佰融】分发失败："+msg));
        return new SendResult(false,"【杭州正佰融】分发失败：{}"+msg);
    }
...
//        ApiSender api = new BaiRongHZApi();
