 ...
 * 北京兴贷
 */
@Component("apiSender_20129")
public class BJXingdaiApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(BJXingdaiApi.class);

    private static final String sendUrl = "https://api.xingdaiqianglian.com/sem/loan_do.html";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[北京兴贷]推送异常：{},{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京兴贷]推送异常:"+e.getMessage()));
            return new SendResult(false,"[北京兴贷]推送异常："+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        if(Long.valueOf(result).longValue() > 1000){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[北京兴贷]推送成功:"+result));
            return new SendResult(true,"[北京兴贷]推送成功："+result);
        }else if("3".equals(result)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京兴贷]推送重复:"+result));
            return new SendResult(false,"[北京兴贷]推送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京兴贷]推送失败:"+result));
        return new SendResult(false,"[北京兴贷]推送失败："+result);
    }
...
//        ApiSender api = new BJXingdaiApi();
