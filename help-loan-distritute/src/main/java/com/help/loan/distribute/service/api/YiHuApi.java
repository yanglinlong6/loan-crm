...
/**
 * 上海翊浒：apiSender_10046   e秒网
 */
@Component("apiSender_10046")
public class YiHuApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(YiHuApi.class);
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            LOG.error("【上海翊浒】分发异常",e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海翊浒】:"+e.getMessage()));
            return new SendResult(false,"【上海翊浒】分发异常："+e.getMessage());
        }
    ...
    private SendResult send2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("【上海翊浒】验证手机号码是否存在：{}",checkResult);
...
//            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【杭州信荣】:"+checkResult));
            return new SendResult(false,"【上海翊浒】手机已存在:"+checkResult);
        }
        ...
        if(0 == jsonData.getIntValue("ResultCode")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海翊浒】发送数据接收失败:"+message));
            return new SendResult(false,"【上海翊浒】发送数据接收失败："+message);
        }else if(2 == jsonData.getIntValue("ResultCode")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【上海翊浒】发送数据数据重复:"+message));
            return new SendResult(false,"【上海翊浒】发送数据数据重复："+message);
        }else if(1 == jsonData.getIntValue("ResultCode")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【上海翊浒】发送数据成功:"+message));
            return new SendResult(true,"【上海翊浒】发送数据成功："+message);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海翊浒】发送数据失败:"+message));
        return new SendResult(false,"【上海翊浒】发送数据失败："+message);
    }
