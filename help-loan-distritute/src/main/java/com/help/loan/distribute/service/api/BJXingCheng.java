 ...
 * 	北京星诚
 */
@Component("apiSender_10016")
public class BJXingCheng implements ApiSender {
    private static final Logger log = LoggerFactory.getLogger(BJXingCheng.class);

    private static final String checkUrl="http://api.liyidai.com/crm/a/check";
    private static final String sendUrl="http://api.liyidai.com/crm/a/postData";

    private static final String providerId="a1c5febb41bb411f90b44090bd20c46a";
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[北京星诚-北京]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京星诚-北京]分发异常:"+e.getMessage()));
            return new SendResult(false,"[北京星诚-北京]分发异常"+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("[北京星诚-北京]撞库检查结果:{}",checkResult);
        if(!JSONUtil.toJSON(checkResult).getBooleanValue("success")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京星诚-北京]撞库重复:"+checkResult));
            return new SendResult(false,"[北京星诚-北京]撞库重复:"+checkResult);
        }
        isHaveAptitude(po);
        data.clear();
...
//        data.put("phoneMd5", MD5Util.getMd5String(po.getMobile()));
        ...
        log.info("[北京星诚-北京]分发结果:{}",result);
        JSONObject json = JSONUtil.toJSON(result);
        if(json.getIntValue("code") == 200){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[北京星诚-北京]分发成功:"+result));
            return new SendResult(true,"[北京星诚-北京]分发成功:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京星诚-北京]分发失败:"+result));
        return new SendResult(false,"[北京星诚-北京]分发失败:"+result);
