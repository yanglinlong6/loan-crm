...
/**
 * 杭州熙南：apiSender_10104
 */
@Component("apiSender_10104")
public class HZXiNanAPI implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(HZXiNanAPI.class);

    String appid = "ffa4c1c0f9c210f9750c33738e3adfad";
    String secret = "db9c8070616a4b73b31ef6a3b1954e2c";
    int  sourceId = 9639;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【杭州熙南】:"+e.getMessage()));
            return new SendResult(false,"【杭州熙南】分发失败");
        }
...
//        param.setRemark("个人公积金月缴500以上，有工作，银行代发5000以上");
        ...
        if(JSONUtil.isJsonString(result) && "0".equals(JSONUtil.toJSON(result).getString("code"))){
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【杭州熙南】手机号已存在"));
        }else
