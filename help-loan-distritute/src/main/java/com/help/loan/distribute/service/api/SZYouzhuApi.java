 ...
 * 苏州优助
 */
@Component("apiSender_20132")
public class SZYouzhuApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(SZYouzhuApi.class);

    private static final String sendUrl = "https://api.youzhu999.com/sem/loan_do.html";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[苏州优助]推送异常：{},{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[苏州优助]推送异常:"+e.getMessage()));
            return new SendResult(false,"[苏州优助]推送异常："+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        if(Long.valueOf(result).longValue() > 1000){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[苏州优助]推送成功:"+result));
            return new SendResult(true,"[苏州优助]推送成功："+result);
        }else if("3".equals(result)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[苏州优助]推送重复:"+result));
            return new SendResult(false,"[苏州优助]推送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[苏州优助]推送失败:"+result));
        return new SendResult(false,"[苏州优助]推送失败："+result);
    }
