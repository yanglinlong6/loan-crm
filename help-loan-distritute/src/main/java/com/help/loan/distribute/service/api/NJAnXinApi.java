...
/**
 * 南京安昕 */
@Component("apiSender_20003")
public class NJAnXinApi implements ApiSender {

    private static Logger LOG = LoggerFactory.getLogger(NJAnXinApi.class);

    private static final Logger log = LoggerFactory.getLogger(NJAnXinApi.class);

    private static final String sendUrl="http://open.fuzejinfu.com/Customer/import";

    private static final String appid = "104207";

    private static final String appkey = "2p02JgWHk207sesdsdaDj0TMw9PZWKK0T7ai";

    private static final String vender_name = "南京安昕";

    private static final String channel = "ddff";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            LOG.error("【南京安昕】分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京安昕】分发异常"));
    ...
    private SendResult send2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("【南京安昕】分发结果：{}",parseObject);
        ...
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),status,"【南京安昕】"+postFormForObject));
    ...
    private String sha1(String param) {
        try {
            ...
            LOG.error("【南京安昕】分发加密异常：{}",e.getMessage(),e);
        }
        return null;
    }
