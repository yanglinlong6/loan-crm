 ...
 *
 */
@Component("apiSender_20078")
public class SHBangYouCaiApi implements ApiSender {


    private static final Logger LOG = LoggerFactory.getLogger(SHBangYouCaiApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "f68d3ebc437a4a2280af3da6930b4998";

    private static Long channelId = 56L;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("[上海帮优财-债务]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海帮优财-债务】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[上海帮优财-债务]分发异常:"+e.getMessage());
        }
    }
    ...
    private SendResult sendResult2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[上海帮优财-债务]客户信息:{}",customerPO.toString());
        ...
        LOG.info("【上海帮优财-债务】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【上海帮优财-债务】分发成功："+result));
            return new SendResult(true,"【上海帮优财-债务】分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【上海帮优财-债务】发送重复："+result));
            return new SendResult(false,"【上海帮优财-债务】发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海帮优财-债务】发送失败："+result));
        return new SendResult(false,"【上海帮优财-债务】发送失败："+result);
    }
    ...
    public static void main(String[] args){
        ...
        ApiSender api = new SHBangYouCaiApi();
