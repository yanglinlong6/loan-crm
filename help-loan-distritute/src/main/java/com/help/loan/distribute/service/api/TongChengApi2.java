 ...
 * 同城api对接
 */
public class TongChengApi2 implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(TongChengApi2.class);

    private static final String sendUrl = "http://tcyd.notify.succtime.com/customerNotify/crmNotify/gogo-hz2";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        ...
        try{
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【同城-杭州】："+e.getMessage()));
            log.error("【同城-杭州】：{}",e.getMessage(),e);
            return new SendResult(false,"【同城-杭州】："+e.getMessage());
        }
    }
