 ...
 * 	北京京贷
 */
@Component("apiSender_20101")
public class BJJingdaiApi implements ApiSender {
    private static final Logger log = LoggerFactory.getLogger(BJJingdaiApi.class);
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[北京京贷]推送异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京京贷]推送异常:"+e.getMessage()));
            return new SendResult(false,"[北京京贷]推送异常"+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select) throws UnsupportedEncodingException {
        ...
        log.info("[北京京贷]推送结果:{}",result);//1471102
        Long code = Long.valueOf(result);
        if(code>1000){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[北京京贷]推送成功:"+result));
            return new SendResult(true,"[北京京贷]推送成功:"+result);
        }
        if(3 == code.intValue()){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京京贷]推送重复:"+result));
            return new SendResult(false,"[北京京贷]推送重复:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京京贷]推送失败:"+result));
        return new SendResult(false,"[北京京贷]推送失败:"+result);
...
//        ApiSender api = new BJJingdaiApi();
