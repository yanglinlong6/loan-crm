 ...
 * 北京商贷
 */
@Component("apiSender_10010")
public class BJShangDaiApi implements ApiSender {
    private static final Logger log = LoggerFactory.getLogger(BJShangDaiApi.class);

    private static final String checkUrl="http://8.131.105.30/api/customer/check_mobile";
    private static final String sendUrl="http://8.131.105.30/api/customer/import_data";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[北京商贷-北京]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京商贷-北京]分发异常:"+e.getMessage()));
            return new SendResult(false,"[北京商贷-北京]分发异常"+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("[北京商贷-北京]撞库检查结果:{}",checkResult);
        if(JSONUtil.toJSON(checkResult).getIntValue("code") != 0){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京商贷-北京]撞库重复:"+checkResult));
            return new SendResult(false,"[北京商贷-北京]撞库重复:"+checkResult);
        }
        isHaveAptitude(po);
        data.clear();
...
//        data.put("phoneMd5", MD5Util.getMd5String(po.getMobile()));
        ...
        log.info("[北京商贷-北京]传输参数:{}",data.toJSONString());
        String result = HttpClientProxy.doPost(sendUrl, data, "UTF-8", 3000, httpHeader);
        log.info("[北京商贷-北京]分发结果:{}",result);
        JSONObject json = JSONUtil.toJSON(result);
        if(json.getIntValue("code") == 0){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[北京商贷-北京]分发成功:"+result));
            return new SendResult(true,"[北京商贷-北京]分发成功:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京商贷-北京]分发失败:"+result));
        return new SendResult(false,"[北京商贷-北京]分发失败:"+result);
...
//        ApiSender api = new BJShangDaiApi();
