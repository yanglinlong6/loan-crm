 ...
 * 易贷中心金融服务外包(北京)有限公司
 */
@Component("apiSender_10009")
public class BJYiDaiZhongXinApi implements ApiSender {
    private static final Logger log = LoggerFactory.getLogger(BJYiDaiZhongXinApi.class);
    ...
    private static final String providerId="7fb57910b73a41a0b155e40cf2242df2";
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[易贷中心-北京]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[易贷中心-北京]分发异常:"+e.getMessage()));
            return new SendResult(false,"[易贷中心-北京]分发异常"+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("[易贷中心-北京]撞库检查结果:{}",checkResult);
        if(!JSONUtil.toJSON(checkResult).getBooleanValue("success")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[易贷中心-北京]撞库重复:"+checkResult));
            return new SendResult(false,"[易贷中心-北京]撞库重复:"+checkResult);
        }
        isHaveAptitude(po);
        data.clear();
...
//        data.put("phoneMd5", MD5Util.getMd5String(po.getMobile()));
        ...
        log.info("[易贷中心-北京]分发结果:{}",result);
        JSONObject json = JSONUtil.toJSON(result);
        if(json.getIntValue("code") == 200){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[易贷中心-北京]分发成功:"+result));
            return new SendResult(true,"[易贷中心-北京]分发成功:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[易贷中心-北京]分发失败:"+result));
        return new SendResult(false,"[易贷中心-北京]分发失败:"+result);
...
//        ApiSender api = new BJYiDaiZhongXinApi();
