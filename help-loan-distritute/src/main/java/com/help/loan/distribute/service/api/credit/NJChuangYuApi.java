...
/**
 * 南京创誉法律咨询有限公司 : 信用卡法律咨询
 *
 */
@Component("apiSender_20068")
public class NJChuangYuApi implements ApiSender {


    private static final Logger LOG = LoggerFactory.getLogger(NJChuangYuApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "da1dff66fe1942efb01f030c896cfb93";

    private static Long channelId = 47l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("[南京创誉法律-信用卡逾期]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京创誉法律-信用卡逾期】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[南京创誉法律-信用卡逾期]分发异常:"+e.getMessage());
        }
    }
    ...
    private SendResult sendResult2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[南京创誉法律]客户信息:{}",customerPO.toString());
        ...
        LOG.info("【南京创誉法律-信用卡逾期】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【南京创誉法律-信用卡逾期】分发成功："+result));
            return new SendResult(true,"【南京创誉法律-信用卡逾期】分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【南京创誉法律-信用卡逾期】发送重复："+result));
            return new SendResult(false,"【南京创誉法律-信用卡逾期】发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京创誉法律-信用卡逾期】发送失败："+result));
        return new SendResult(false,"【南京创誉法律-信用卡逾期】发送失败："+result);
    }
    ...
    public static class CryptAES {
        ...
        public static String AES_Encrypt(String keyStr, String plainText) {
            ...
            try{
                ...
                LOG.error("【南京创誉法律-信用卡逾期】AES加密异常:{}",e.getMessage(),e);
            }
        ...
        public static String AES_Decrypt(String keyStr, String encryptData) {
            ...
            try{
                ...
                LOG.error("【南京创誉法律-信用卡逾期】AES解密异常:{}",e.getMessage(),e);
            }
...
//        ApiSender api = new NJChuangYuApi();
