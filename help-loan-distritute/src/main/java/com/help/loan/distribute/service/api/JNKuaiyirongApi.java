 ...
 * 济南快易融
 */
@Component("apiSender_20095")
public class JNKuaiyirongApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(JNKuaiyirongApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "8d0fade31f1c4cbe836e5ebfbbfbf3bc";

    private static Long channelId = 61l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("[济南快易融]分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[济南快易融]分发异常:"+e.getMessage()));
            return new SendResult(false,"[济南快易融]分发异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[济南快易融]发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[济南快易融]分发成功："+result));
            return new SendResult(true,"[济南快易融]分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[济南快易融]发送重复："+result));
            return new SendResult(false,"[济南快易融]发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[济南快易融]发送失败："+result));
        return new SendResult(false,"[济南快易融]发送失败："+result);
    }
