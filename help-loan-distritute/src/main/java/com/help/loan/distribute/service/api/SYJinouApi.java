...
import com.help.loan.distribute.service.api.credit.NJChuangYuApi;
 ...
 * 沈阳金鸥经济信息咨询有限公司-房抵
 */
@Component("apiSender_20086")
public class SYJinouApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(SYJinouApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "44aaa930c845476d8138bc603f485427";

    private static Long channelId = 60l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("【沈阳金鸥-房抵】分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【沈阳金鸥-房抵】分发异常:"+e.getMessage()));
            return new SendResult(false,"【沈阳金鸥-房抵】分发异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("【沈阳金鸥-房抵】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【沈阳金鸥-房抵】分发成功："+result));
            return new SendResult(true,"【沈阳金鸥-房抵】分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【沈阳金鸥-房抵】发送重复："+result));
            return new SendResult(false,"【沈阳金鸥-房抵】发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【沈阳金鸥-房抵】发送失败："+result));
        return new SendResult(false,"【沈阳金鸥-房抵】发送失败："+result);
    }
