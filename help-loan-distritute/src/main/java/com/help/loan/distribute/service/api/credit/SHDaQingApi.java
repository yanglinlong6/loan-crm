...
/**
 * 上海达清法律咨询有限公司 : 信用卡法律咨询
 *
 */
@Component("apiSender_20071")
public class SHDaQingApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(SHDaQingApi.class);

    private static final Logger LOG = LoggerFactory.getLogger(SHDaQingApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "872ddb4aadbe4904a8cb5a6890f8977c";

    private static Long channelId = 51l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[上海达清法律-信用卡逾期]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海达清法律-信用卡逾期】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[上海达清法律-信用卡逾期]分发异常:"+e.getMessage());
        }
    }
    ...
    private SendResult sendResult2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[上海达清法律]客户信息:{}",customerPO.toString());
        ...
        LOG.info("【上海达清法律-信用卡逾期】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【上海达清法律-信用卡逾期】分发成功："+result));
            return new SendResult(true,"【上海达清法律-信用卡逾期】分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【上海达清法律-信用卡逾期】发送重复："+result));
            return new SendResult(false,"【上海达清法律-信用卡逾期】发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海达清法律-信用卡逾期】发送失败："+result));
        return new SendResult(false,"【上海达清法律-信用卡逾期】发送失败："+result);
    }
