 ...
 * 沈阳盛世
 */
@Component("apiSender_20116")
public class SHYShengshiApi implements ApiSender{

    private static final Logger LOG = LoggerFactory.getLogger(SHYShengshiApi.class);

    private static final String key = "fluxZhudai*_key";

    /**租户id*/
    private static final String tenanId = "1566665238734966786";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[沈阳盛世]推送未知异常："+e.getMessage()));
            LOG.error("[沈阳盛世]推送异常:{}-{}",e.getMessage(),e);
            return new SendResult(false,"[沈阳盛世]推送异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[沈阳盛世]撞库重复："+result));
        return new SendResult(false,"[沈阳盛世]撞库重复:"+result);
    }
    private SendResult register(UserAptitudePO po, UserDTO select){
        ...
        customer.put("provinceCode",210000); // 上海市
        customer.put("cityCode",210100);//上海市
        JSONArray customers = new JSONArray();
        ...
        if(200 == code
                && 0 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[沈阳盛世]推送成功："+result));
            return new SendResult(true,"[沈阳盛世]推送成功:"+result);
        }
        if(200 == code
                && 1 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[沈阳盛世]推送重复："+result));
            return new SendResult(false,"[沈阳盛世]推送重复:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[享贷科技]推送失败："+result));
        return new SendResult(false,"[沈阳盛世]推送失败:"+result);
    }
...
//
//        ApiSender apiSender = new SHYShengshiApi();
