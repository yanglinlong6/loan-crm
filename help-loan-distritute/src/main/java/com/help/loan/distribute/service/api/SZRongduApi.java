...
/**
 * 苏州融度信息科技有限公司 apiSender_20045
 */
@Component("apiSender_20045")
public class SZRongduApi implements ApiSender{

    private static final Logger log = LoggerFactory.getLogger(SZRongduApi.class);
    ...
    private static final String tenantId = "472843";

    private static final String channelCode = "92296898";

    private static final String cluePoolCode = "58523681";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[苏州融度信息]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【苏州融度信息】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[苏州融度信息]分发异常:"+e.getMessage());
        }
    }
...
//        String checkResult = HttpUtil.postForObject(checkUrl,data);
        ...
        if("duplicate".equals(json.getString("data"))){
            log.info("[苏州融度信息]撞库-重复:{}",checkResult);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【苏州融度信息】撞库重复："+checkResult));
            return new SendResult(false,"[苏州融度信息]撞库结果-重复: "+checkResult);
        }
        data.clear();
        data.put("tenantId",tenantId);
        ...
        log.info("【苏州融度信息】分发结果：{}",sendResult);
        int code = JSONUtil.toJSON(sendResult).getIntValue("code");
        if(200==code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【苏州融度信息】http表单分发成功："+sendResult));
            return new SendResult(true,"【苏州融度信息】分发成功：{}"+sendResult);
        }else if(301 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【苏州融度信息】分发重复："+sendResult));
            return new SendResult(false,"【苏州融度信息】分发重复：{}"+sendResult);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【苏州融度信息】分发失败："+sendResult));
        return new SendResult(false,"【苏州融度信息】分发失败：{}"+sendResult);
    }
