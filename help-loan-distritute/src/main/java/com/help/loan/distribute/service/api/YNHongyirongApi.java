 ...
 * 云南弘易融商务信息服务有限公司
 */
@Component("apiSender_20135")
public class YNHongyirongApi implements ApiSender{

    private static final Logger LOG = LoggerFactory.getLogger(YNHongyirongApi.class);

    private static final String key = "fluxZhudai*_key";

    /**租户id*/
    private static final String tenanId = "1630851137416491010";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[云南弘易融]推送未知异常："+e.getMessage()));
            LOG.error("[云南弘易融]推送异常:{}-{}",e.getMessage(),e);
            return new SendResult(false,"[云南弘易融]推送异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[云南弘易融]撞库重复："+result));
        return new SendResult(false,"[云南弘易融]撞库重复:"+result);
    }
    private SendResult register(UserAptitudePO po, UserDTO select){
        ...
        customer.put("provinceCode",530000); // 上海市
        customer.put("cityCode",530100);//上海市
        JSONArray customers = new JSONArray();
        ...
        if(200 == code
                && 0 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[云南弘易融]推送成功："+result));
            return new SendResult(true,"[云南弘易融]推送成功:"+result);
        }
        if(200 == code
                && 1 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[云南弘易融]推送重复："+result));
            return new SendResult(false,"[云南弘易融]推送重复:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[享贷科技]推送失败："+result));
        return new SendResult(false,"[云南弘易融]推送失败:"+result);
    }
...
//
//        ApiSender apiSender = new YNHongyirongApi();
