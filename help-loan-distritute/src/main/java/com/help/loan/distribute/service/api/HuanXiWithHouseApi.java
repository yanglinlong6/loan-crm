...
/*
 *  欢喜房抵车抵 apiSender_20031
 */
@Component("apiSender_20031")
public class HuanXiWithHouseApi implements ApiSender {
    private static final Logger log = LoggerFactory.getLogger(HuanXiWithHouseApi.class);
...
//    private static final String src_car = "bzcd01";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            if(!JudgeUtil.in(po.getCreateBy(),DisConstant.User.Type.HOUSE)){
                return new SendResult(false,"不是房抵客户："+po.getCreateBy());
            }
            return sendResult(po,select);
        }catch (Exception e){
            log.error("[欢喜-车抵or房抵]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[欢喜-车抵or房抵]分发异常:"+e.getMessage()));
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select) throws IOException {
        ...
        String v = MD5Util.getMd5String(po.getMobile()+cid_house+key);
        String url = String.format(sendUrl,cid_house,src_house,v);
...
//        ApiSender api = new HuanXiWithHouseApi();
