 ...
 *  北京鑫盈 20155
 */
@Component("apiSender_20155")
public class XinYingForBeijingApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(XinYingForBeijingApi.class);
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            log.error("[北京鑫盈]推送异常:{}", e.getMessage(), e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京鑫盈]推送异常:" + e.getMessage()));
            return new SendResult(false, "[北京鑫盈]推送异常:" + e.getMessage());
        }
    }
    ...
    private SendResult sendResultForJinan(UserAptitudePO po, UserDTO select) {
        ...
        String checkResult = HttpUtil.postFormForObject("https://api.daizhu8.com/sem_oo/check_loan.html", checkData);
        log.info("[北京鑫盈]撞库结果:{}", checkResult);
        if (JSONUtil.toJSON(checkResult).getIntValue("Code") != 100) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[北京鑫盈]推送重复:" + checkResult));
            return new SendResult(false, "[北京鑫盈]撞库重复:" + checkResult);
        }
        isHaveAptitude(po);
        ...
        String url = "https://api.daizhu8.com/sem/loan_do.html";
        String result = HttpUtil.postFormForObject(url, data);
        log.info("[北京鑫盈]推送结果:{}", result);
        if (Long.parseLong(result) > 1000) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 1, "[北京鑫盈]推送成功:" + result));
            return new SendResult(true, "[北京鑫盈]推送成功：" + result);
        } else if ("3".equals(result)) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[北京鑫盈]推送重复:" + result));
            return new SendResult(false, "[北京鑫盈]推送重复：" + result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[北京鑫盈]推送失败:" + result));
        return new SendResult(false, "[北京鑫盈]推送失败：" + result);
    }
...
//        po.setUpdateDate(new Date());
        ...
        po.setUpdateDate(new Date());
        ApiSender api = new XinYingForBeijingApi();
