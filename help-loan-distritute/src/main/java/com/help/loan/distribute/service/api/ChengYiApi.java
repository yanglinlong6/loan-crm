 ...
 * 北京诚毅商务信息咨询有限公司：
 */
@Component("apiSender_10006")
public class ChengYiApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(ChengYiApi.class);

    private static final String checkUrl = "https://hzapi.zgzdzx.cn/API/CustExists.ashx";

    private static final String sendUrl = "https://hzapi.zgzdzx.cn/API/Customer.ashx";

    private static final String checkUrlT = "https://apitest.zgzdzx.cn/API/CustExists.ashx";

    private static final String sendUrlT = "https://apitest.zgzdzx.cn/API/Customer.ashx";
    
    private static final Integer channle = 100017;

    private static final String key = "Fhahkjdhjfkhhf3k5x";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            LOG.error("【北京诚毅】分发异常",e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【北京诚毅】:"+e.getMessage()));
            return new SendResult(false,"【北京诚毅】分发异常："+e.getMessage());
        }
    ...
    private SendResult send2(UserAptitudePO po, UserDTO select){
        ...
        String msg = JSONUtil.toJSON(checkResult).getString("ResultMsg");
        LOG.info("【北京诚毅】验证手机号码是否存在：{}",msg);
        if(JSONUtil.isJsonString(checkResult) && 1 != JSONUtil.toJSON(checkResult).getIntValue("ResultCode")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【北京诚毅】:"+checkResult));
            return new SendResult(false,"【北京诚毅】撞库:"+msg);
        }
        ...
        LOG.info("【北京诚毅】发送数据结果：{}",result);
        if(!JSONUtil.isJsonString(result)){
            LOG.info("【北京诚毅】发送数据失败：{},返回格式错误",result);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【上海翊浒】发送数据返回格式错误:"+checkResult));
            return new SendResult(false,"【北京诚毅】发送数据返回格式错误："+result);
        }
        ...
        if(0 == jsonData.getIntValue("ResultCode")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【北京诚毅】发送数据接收失败:"+message));
            return new SendResult(false,"【北京诚毅】发送数据接收失败："+message);
        }else if(2 == jsonData.getIntValue("ResultCode")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【北京诚毅】发送数据数据重复:"+message));
            return new SendResult(false,"【北京诚毅】发送数据数据重复："+message);
        }else if(1 == jsonData.getIntValue("ResultCode")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【北京诚毅】发送数据成功:"+message));
            return new SendResult(true,"【北京诚毅】发送数据成功："+message);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【北京诚毅】发送数据失败:"+message));
        return new SendResult(false,"【北京诚毅】发送数据失败："+message);
    }
