 ...
 * 金闪借：apiSender_10035
 */
@Component("apiSender_10035")
public class JinShaJieAPI implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(JinShaJieAPI.class);

    String appid = "de52910a4edd5f00e1988fa16cabe508";
    String secret = "0b9725d86dcb4a0d992d6e870669f905";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【金闪借】:"+e.getMessage()));
            return new SendResult(false,"【金闪借】分发失败");
        }
...
//        param.setRemark("个人公积金月缴500以上，有工作，银行代发5000以上");
        ...
        if(JSONUtil.isJsonString(result) && "0".equals(JSONUtil.toJSON(result).getString("code"))){
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【金闪借】手机号已存在"));
        }else
