 ...
 */
public class SHYixunApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(SHYixunApi.class);
    ...
    private static final String tenantId = "845747";

    private static final String toutiaoChannelCode = "62347739";

    private static final String WechartChnannelCode = "14054772";

    private static final String cluePoolCode = "60614552";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            if(StringUtils.isNotBlank(po.getChannel()) && po.getChannel().contains("ttt"))
                return new SendResult(false,"[上海宜寻]暂不接收头条客户");
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select) {
        ...
        if ("duplicate".equals(json.getString("data"))) {
            log.info("[上海宜寻]撞库-重复:{}", checkResult);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "【上海宜寻】撞库重复：" + checkResult));
            return new SendResult(false, "[上海宜寻]撞库结果-重复: " + checkResult);
        }
        data.clear();
        data.put("tenantId", tenantId);
