 ...
 * 北京融贷网房地产经纪有限公司
 */
@Component("apiSender_20126")
public class BJRongdaiwangIIIApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(BJRongdaiwangIIIApi.class);
    
    private static final String URL = "http://39.107.86.1/crm/import/customer";

    private static String KEY = "a320b3558dd64533b46a3cee6b1ab61d";

    private static Long channelId = 136L;

    private static Long channelId_360 = 136L;

    private static String key_360 = "a320b3558dd64533b46a3cee6b1ab61d";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京融贷网]推送未知异常："+e.getMessage()));
            return new SendResult(false,"[北京融贷网]推送异常："+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[北京融贷网]推送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            if(!JudgeUtil.startWith(po.getChannel(),"360")){
                dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[北京融贷网]推送成功："+result));
            }
            return new SendResult(true,"[北京融贷网]推送成功："+result);
        }else if(601 == code){
            if(!JudgeUtil.startWith(po.getChannel(),"360")){
                dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京融贷网]推送重复："+result));
            }
            return new SendResult(false,"[北京融贷网]推送重复："+result);
        }
        if(!JudgeUtil.startWith(po.getChannel(),"360")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京融贷网]推送失败："+result));
        }
        return new SendResult(false,"[北京融贷网]推送失败："+result);
    }
...
//        ApiSender api = new BJRongdaiwangIIIApi();
