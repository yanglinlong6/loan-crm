...
/**
 * 武汉市英茂商务咨询有限公司：apiSender_20041
 */
@Component("apiSender_20041")
public class YingmaoApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(YingmaoApi.class);

    private static final String venderName = "DF";

    private static final  String appid = "21080";

    private static final String appkey = "oDOX7KY0yjWB7tOE";

    private static final String checkUrl = "http://114.55.25.96:8023/openapi/customer/check_master";

    private static final String url = "http://114.55.25.96:8023/openapi/customer/import_v2";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            if(1 != JSONUtil.toJSON(result).getIntValue("resStatus")){
                dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【武汉振为】:"+result));
                return new SendResult(false,"【武汉振为】重复："+result);
            }
            return send2(po,select);
        }catch (Exception e){
            LOG.error("[武汉振为]发送异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【武汉振为】:"+e.getMessage()));
...
//        data.put("signature",sha1(signString.toString()));
        ...
        if(1==status){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【武汉振为】成功:"+resMsg));
            return new SendResult(true,resMsg);
        }else if(2 == status){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【武汉振为】重复:"+resMsg));
            return new SendResult(false,"【武汉振为】重复："+resMsg);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【武汉振为】:"+resMsg));
        return new SendResult(false,"【武汉振为】失败："+resMsg);
    }
