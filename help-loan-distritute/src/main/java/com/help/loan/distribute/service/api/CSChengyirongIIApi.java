...
/**
 * 诚易融-II 17844620375  1469292409@qq.com
 */
@Component("apiSender_20131")
public class CSChengyirongIIApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(CSChengyirongIIApi.class);

    private static final String URL = "https://crm.sykj888.cn/crm/import/customer";

    private static String KEY = "6a52c8eb5349486092a2e70e4483fe9e";

    private static Long CHENNEL = 160l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("[诚易融-II]推送异常：{}",e.getMessage(),e);
            if(!JudgeUtil.startWith(po.getChannel(),"360")){
                dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[诚易融-II]推送异常:"+e.getMessage()));
            }
            return new SendResult(false,"[诚易融-II]推送异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[诚易融-II]推送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[诚易融-II]推送成功："+result));
            return new SendResult(true,"[诚易融-II]推送成功："+result);
        }else if(601 == code){
            if(!JudgeUtil.startWith(po.getChannel(),"360","xinxiangrong")){
                dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[诚易融-II]推送重复："+result));
            }
            return new SendResult(false,"[诚易融-II]推送重复："+result);
        }
        if(!JudgeUtil.startWith(po.getChannel(),"360","xinxiangrong")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[诚易融-II]推送失败："+result));
        }
        return new SendResult(false,"[诚易融-II]推送失败："+result);
    }
...
//        ApiSender api = new CSChengyirongIIApi();
