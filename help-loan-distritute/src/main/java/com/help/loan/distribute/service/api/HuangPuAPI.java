...
/**
 * 杭州黄埔：apiSender_10067
 */
@Component("apiSender_10067")
public class HuangPuAPI implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(HuangPuAPI.class);

    String appid = "ab2fc942a3323282040e0a2ce4b0af51";
    String secret = "aef73383022b40b59930683e0aeecfd4";
    int  sourceId = 9630;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【杭州黄埔】:"+e.getMessage()));
            return new SendResult(false,"【杭州黄埔】分发失败");
        }
...
//        param.setRemark("个人公积金月缴500以上，有工作，银行代发5000以上");
        ...
        if(JSONUtil.isJsonString(result) && "0".equals(JSONUtil.toJSON(result).getString("code"))){
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【杭州黄埔】手机号已存在"));
        }else
