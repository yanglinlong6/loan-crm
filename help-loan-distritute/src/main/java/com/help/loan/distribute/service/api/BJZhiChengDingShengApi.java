 ...
 * 北京-至诚鼎盛
 */
@Component("apiSender_20026")
public class BJZhiChengDingShengApi implements ApiSender{

    private static final Logger log = LoggerFactory.getLogger(BJZhiChengDingShengApi.class);

    private static final String checkUrl = "http://112.126.75.7:8200/customers?encrypt_phone=";

    private static final String sendUrl = "http://112.126.75.7:8200/customers";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("【至诚鼎盛-北京】分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【至诚鼎盛-北京】分发未知异常："+e.getMessage()));
            return new SendResult(false,"【至诚鼎盛-北京】分发异常:"+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("【至诚鼎盛-北京】撞库结果：{}",result);
        if(0 != JSONUtil.toJSON(result).getIntValue("code")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【至诚鼎盛-北京】分发未知异常："+result));
            return new SendResult(false,"【至诚鼎盛-北京】客户已存在："+result);
        }
        ...
        log.info("[至诚鼎盛-北京]分发结果:{}",result);
        String code = JSONUtil.toJSON(result).getString("code");
        if(code.equals("0")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[至诚鼎盛-北京]分发成功:"+result));
            return new SendResult(true,"[至诚鼎盛-北京]发送成功:"+result);
        }
        //{"code":"1","msg":"号码已存在"}
        if(JSONUtil.toJSON(result).getString("msg").equals("has exist")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[至诚鼎盛-北京]分发重复:"+result));
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[至诚鼎盛-北京]分发失败:"+result));
        return new SendResult(false,"[至诚鼎盛-北京]发送失败:"+result);
    }
    ...
    public static void main(String[] args){
        ...
        ApiSender api = new BJZhiChengDingShengApi();
