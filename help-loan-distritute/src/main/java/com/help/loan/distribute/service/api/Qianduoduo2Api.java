...
@Component("apiSender_10069")
public class Qianduoduo2Api implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(Qianduoduo2Api.class);
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("【钱多多[2]分发异常：{}】",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【钱多多-2】分发异常:"+e.getMessage()));
            return new SendResult(false,"钱多多[2]分发异常："+e.getMessage());
        }
    ...
    private SendResult send2(UserAptitudePO po, UserDTO select){
        ...
        log.info("【钱多多-2】分发结果：{}",response);
        JSONObject json = JSONUtil.toJSON(response);
        int code = json.getIntValue("code");
        if(code == 0){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【钱多多-2】分发成功:"+response));
            return new SendResult(true,"【钱多多-2】分发成功："+response);
        }
        if(code == 3){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【钱多多-2】分发重复:"+response));
            return new SendResult(false,"【钱多多-2】分发重复："+response);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【钱多多-2】分发失败:"+response));
        return new SendResult(false,"【钱多多-2】分发失败："+response);
    }
...
//        Qianduoduo2Api api = new Qianduoduo2Api();
