...
/**
 * 首金贷款代理(东莞)有限公司：apiSender_10120
 */
@Component("apiSender_10120")
public class ZhuHaiShouJInAPI implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(ZhuHaiShouJInAPI.class);

    String appid = "c41da3f0e8addf3fdb367880287ddfba";
    String secret = "13263513246d4b608e0d91b32765bfc7";
    int  sourceId = 9642;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【珠海首金贷款】:"+e.getMessage()));
            return new SendResult(false,"【珠海首金贷款】分发失败");
        }
    ...
    private SendResult send3(UserAptitudePO po, UserDTO select){
        ...
        if(JSONUtil.isJsonString(result) && "0".equals(JSONUtil.toJSON(result).getString("code"))){
            ...
            return new SendResult(true,"[珠海首金贷款]分发成功:"+result);
        }else if("000001".equals(JSONUtil.toJSON(result).getString("code"))){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【珠海首金贷款】手机号已存在"));
        }else
...
//        ApiSender api = new ZhuHaiShouJInAPI();
