 ...
 * 沃得财富金融服务外包(北京)有限公司
 */
@Component("apiSender_10007")
public class BJWodeApi implements ApiSender {
    private static final Logger log = LoggerFactory.getLogger(BJWodeApi.class);

    private static final String checkUrl="http://wd.crm.rcplay.cn/crm/a/check";
    private static final String sendUrl="http://wd.crm.rcplay.cn/crm/a/postData";

    private static final String providerId="69b86e2bafae404eaefaf6b220bbac12";
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[沃得财富-北京]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[沃得财富-北京]分发异常:"+e.getMessage()));
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("[沃得财富-北京]撞库检查结果:{}",checkResult);
        if(!JSONUtil.toJSON(checkResult).getBooleanValue("success")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[沃得财富-北京]撞库重复:"+checkResult));
            return new SendResult(false,"[沃得财富-北京]撞库重复:"+checkResult);
        }
        isHaveAptitude(po);
        data.clear();
...
//        data.put("phoneMd5", MD5Util.getMd5String(po.getMobile()));
        ...
        log.info("[沃得财富-北京]分发结果:{}",result);
        JSONObject json = JSONUtil.toJSON(result);
        if(json.getIntValue("code") == 200){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[沃得财富-北京]分发成功:"+result));
            return new SendResult(true,"[沃得财富-北京]分发成功:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[沃得财富-北京]分发失败:"+result));
        return new SendResult(false,"[沃得财富-北京]分发失败:"+result);
