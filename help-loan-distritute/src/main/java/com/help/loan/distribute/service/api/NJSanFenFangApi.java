 ...
 * 南京小微-房抵-房抵
 */
@Component("apiSender_20059")
public class NJSanFenFangApi implements ApiSender{
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            return new SendResult(false,"【南京小微-房抵】非房抵客户");
        }catch (Exception e){
            LOG.error("【南京小微-房抵】分发异常：{}",e.getMessage(),e);
    ...
    private SendResult send2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("【南京小微-房抵】撞库结果：{}",result);
        if(!"0".equals(result)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【南京小微-房抵】撞库重复:"+result));
            return new SendResult(false,"【南京小微-房抵】撞库重复："+result);
        }
        isHaveAptitude(po);
        ...
        LOG.info("【南京小微-房抵】分发结果：{}",result);
        if("0".equals(result)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【南京小微-房抵】分发成功:"+result));
            return new SendResult(true,"【南京小微-房抵】分发成功："+result);
        }else if("3".equals(result)){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【南京小微-房抵】分发重复:"+result));
            return new SendResult(false,"【南京小微-房抵】分发重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京小微-房抵】分发失败:"+result));
        return new SendResult(false,"【南京小微-房抵】分发失败："+result);
    }


//    public static void main(String[] args){
//
//        NJSanFenFangApi api = new NJSanFenFangApi();
//
