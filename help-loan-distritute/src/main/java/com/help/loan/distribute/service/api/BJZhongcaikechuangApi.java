 ...
 * 中财科创（北京）金融服务外包有限公司
 */
@Component("apiSender_10090")
public class BJZhongcaikechuangApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(BJZhongcaikechuangApi.class);
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("【北京中财科创】分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【北京中财科创】分发异常:"+e.getMessage()));
            return new SendResult(false,"【北京中财科创】分发异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        if(!"0".equals(JSONUtil.toJSON(checkResult).getString("code"))){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【北京中财科创】分发重复:"+checkResult));
            return new SendResult(false,"【北京中财科创】重复："+checkResult);
        }
...
//        data.put("carDesc",token);
        ...
        log.info("【北京中财科创】分发结果：{}",result);
        JSONObject json = JSONUtil.toJSON(result);
        if("0".equals(json.getString("code"))){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【北京中财科创】分发成功:"+checkResult));
            return new SendResult(true,"【北京中财科创】分发成功："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【北京中财科创】失败:"+checkResult));
        return new SendResult(false,"【北京中财科创】分发失败："+result);
    }
