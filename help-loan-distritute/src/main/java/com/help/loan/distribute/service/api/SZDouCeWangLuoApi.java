 ...
 * 苏州抖策网络科技有限公司
 */
@Component("apiSender_20077")
public class SZDouCeWangLuoApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(SZDouCeWangLuoApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "b22a7a798fbb429986144b669e491d74";

    private static Long channelId = 55l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("【苏州抖策网络】推送异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【苏州抖策网络】推送异常:"+e.getMessage()));
            return new SendResult(false,"【苏州抖策网络】推送异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("【苏州抖策网络】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【苏州抖策网络】推送成功："+result));
            return new SendResult(true,"【苏州抖策网络】推送成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【苏州抖策网络】推送重复："+result));
            return new SendResult(false,"【苏州抖策网络】推送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【苏州抖策网络】推送失败："+result));
        return new SendResult(false,"【苏州抖策网络】推送失败："+result);
    }
...
//        ApiSender api = new SZDouCeWangLuoApi();
