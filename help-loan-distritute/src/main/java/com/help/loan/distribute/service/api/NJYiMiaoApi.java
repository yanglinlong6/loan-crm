 ...
 * 南京一秒
 */
@Component("apiSender_20049")
public class NJYiMiaoApi implements ApiSender{

    private static final Logger log = LoggerFactory.getLogger(NJYiMiaoApi.class);
    ...
    private static final String tenantId = "912657";

    private static final String channelCode = "87685543";

    private static final String cluePoolCode = "73571293";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            return new SendResult(false,"[南京一秒]不接收朋友圈客户");
        }catch (Exception e){
            log.error("[南京一秒]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京一秒】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[南京一秒]分发异常:"+e.getMessage());
        }
    }
...
//        String checkResult = HttpUtil.postForObject(checkUrl,data);
        log.info("[南京一秒]撞库结果 : {}",checkResult);
        JSONObject json = JSONUtil.toJSON(checkResult);
        if("duplicate".equals(json.getString("data"))){
            log.info("[南京一秒]撞库-重复:{}",checkResult);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【南京一秒】撞库重复："+checkResult));
            return new SendResult(false,"[南京一秒]撞库结果-重复: "+checkResult);
        }
        data.clear();
        data.put("tenantId",tenantId);
        ...
        log.info("【南京一秒】分发结果：{}",sendResult);
        int code = JSONUtil.toJSON(sendResult).getIntValue("code");
        if(200==code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【南京一秒】http表单分发成功："+sendResult));
            return new SendResult(true,"【南京一秒】分发成功：{}"+sendResult);
        }else if(301 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【南京一秒】分发重复："+sendResult));
            return new SendResult(false,"【南京一秒】分发重复：{}"+sendResult);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【南京一秒】分发失败："+sendResult));
        return new SendResult(false,"【南京一秒】分发失败：{}"+sendResult);
    }
