 ...
 *
 */
@Component("apiSender_20070")
public class BeiJingJinYanApi implements ApiSender {


    private static final Logger LOG = LoggerFactory.getLogger(BeiJingJinYanApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "dcbab1ae42ae4e47be44f81c4690906b";

    private static Long channelId = 50l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("[北京谨言-信用卡逾期]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【北京谨言-信用卡逾期】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[北京谨言-信用卡逾期]分发异常:"+e.getMessage());
        }
    }
    ...
    private SendResult sendResult2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[北京谨言]客户信息:{}",customerPO.toString());
        ...
        LOG.info("【北京谨言-信用卡逾期】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【北京谨言-信用卡逾期】分发成功："+result));
            return new SendResult(true,"【北京谨言-信用卡逾期】分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【北京谨言-信用卡逾期】发送重复："+result));
            return new SendResult(false,"【北京谨言-信用卡逾期】发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【北京谨言-信用卡逾期】发送失败："+result));
        return new SendResult(false,"【北京谨言-信用卡逾期】发送失败："+result);
    }
    ...
    public static class CryptAES {
        ...
        public static String AES_Encrypt(String keyStr, String plainText) {
            ...
            try{
                ...
                LOG.error("【北京谨言-信用卡逾期】AES加密异常:{}",e.getMessage(),e);
            }
        ...
        public static String AES_Decrypt(String keyStr, String encryptData) {
            ...
            try{
                ...
                LOG.error("【北京谨言-信用卡逾期】AES解密异常:{}",e.getMessage(),e);
            }
