 ...
 * 杭州银贷
 */
@Component("apiSender_20022")
public class HZYinDaiApi implements ApiSender{

    private static final Logger log = LoggerFactory.getLogger(HZYinDaiApi.class);

    private static final String checkUrl = "http://121.36.92.202:8800/customers?encrypt_phone=";

    private static final String sendUrl = "http://121.36.92.202:8800/customers";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("【杭州银贷】分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【杭州银贷】分发未知异常："+e.getMessage()));
            return new SendResult(false,"【杭州银贷】分发异常:"+e.getMessage());
        }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        log.info("【杭州银贷】撞库结果：{}",result);
        if(0 == JSONUtil.toJSON(result).getIntValue("code") &&  "1".equals(JSONUtil.toJSON(result).getString("data"))){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【杭州银贷】分发未知异常："+result));
            return new SendResult(false,"【杭州银贷】客户已存在："+result);
        }
        ...
        log.info("[杭州银贷]分发结果:{}",result);
        String code = JSONUtil.toJSON(result).getString("code");
        if(code.equals("0")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[杭州银贷]分发成功:"+result));
            return new SendResult(true,"[杭州银贷]发送成功:"+result);
        }
        //{"code":"1","msg":"号码已存在"}
        if(JSONUtil.toJSON(result).getString("msg").equals("has exist")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[杭州银贷]分发重复:"+result));
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[杭州银贷]分发失败:"+result));
        return new SendResult(false,"[杭州银贷]发送失败:"+result);
    }
