 ...
 *  北京鑫e融
 */
@Component("apiSender_20057")
public class BJXinERongApi implements ApiSender{

    private static final Logger log = LoggerFactory.getLogger(BJXinERongApi.class);
    private static final String sendUrl="http://crm.52juxin.com/index.php?s=/api/index/add";
    private static final int wxapp_id=10001;
    private static final int channel_id=26;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[北京鑫E融]推送异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京鑫E融]推送异常:"+e.getMessage()));
            return new SendResult(false,"[北京鑫E融]推送异常:"+e.getMessage());
        }
...
//        {"code":1,"msg":"SUCCESS","data":""}
        ...
        log.info("[北京鑫E融]推送结果:{}",result);
        JSONObject json = JSONUtil.toJSON(result);
        int code = json.getIntValue("code");
        if(1 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[北京鑫E融]推送成功:"+result));
            return new SendResult(true,"[北京鑫E融]发送成功:"+result);
        }
        String msg = json.getString("msg");
        if(msg.contains("号码已存在")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[北京鑫E融]推送重复:"+result));
            return new SendResult(false,"[北京鑫E融]推送重复:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[北京鑫E融]推送失败:"+result));
        return new SendResult(false,"[北京鑫E融]发送失败:"+result);
    }
...
//        BJXinERongApi api = new BJXinERongApi();
