 ...
 * 筑帆金融信息服务（上海）有限公司
 */
@Component("apiSender_20115")
public class SHZhufanApi implements ApiSender{

    private static final Logger LOG = LoggerFactory.getLogger(SHZhufanApi.class);

    private static final String key = "fluxZhudai*_key";

    /**租户id*/
    private static final String tenanId = "1453564589617229826";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[上海筑帆]推送未知异常："+e.getMessage()));
            LOG.error("[上海筑帆]推送异常:{}-{}",e.getMessage(),e);
            return new SendResult(false,"[上海筑帆]推送异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[上海筑帆]撞库重复："+result));
        return new SendResult(false,"[上海筑帆]撞库重复:"+result);
    }
    private SendResult register(UserAptitudePO po, UserDTO select){
        ...
        if(200 == code
                && 0 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"[上海筑帆]推送成功："+result));
            return new SendResult(true,"[上海筑帆]推送成功:"+result);
        }
        if(200 == code
                && 1 == customerJson.getIntValue("status")){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"[上海筑帆]推送重复："+result));
            return new SendResult(false,"[上海筑帆]推送重复:"+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"[享贷科技]推送失败："+result));
        return new SendResult(false,"[上海筑帆]推送失败:"+result);
    }
    ...
    public static void main(String[] args){
        ...
        ApiSender apiSender = new SHZhufanApi();
