 ...
 *  苏州优助信息技术有限公司 20145
 */
@Component("apiSender_20145")
public class YouZhuInformationForSuZhou implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(YouZhuInformationForSuZhou.class);
    @Autowired
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            ...
            log.error("[苏州优助信息技术有限公司]推送异常:{}", e.getMessage(), e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[苏州优助信息技术有限公司]推送异常:" + e.getMessage()));
            return new SendResult(false, "[苏州优助信息技术有限公司]推送异常:" + e.getMessage());
        }
    }
...
//        log.info("[苏州优助信息技术有限公司]撞库结果:{}", checkResult);
//        if (JSONUtil.toJSON(checkResult).getIntValue("Code") != 100) {
//            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "[苏州优助信息技术有限公司]推送重复:" + checkResult));
//            return new SendResult(false, "[苏州优助信息技术有限公司]撞库重复:" + checkResult);
//        }
        isHaveAptitude(po);
        ...
        String url = "https://api.youzhu999.com/sem_yf/loan_into.html";
//        String result = HttpUtil.postFormForObject(url, data);
        String result = HttpUtil.postForJSON(url, data);
        log.info("[苏州优助信息技术有限公司]推送结果:{}", result);
        if (JSONUtil.toJSON(result).getIntValue("code") == 200) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 1, "[苏州优助信息技术有限公司]推送成功:" + result));
            return new SendResult(true, "[苏州优助信息技术有限公司]推送成功：" + result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "[苏州优助信息技术有限公司]推送失败:" + result));
        return new SendResult(false, "[苏州优助信息技术有限公司]推送失败：" + result);
    }
...
//        po.setUpdateDate(new Date());
        ...
        po.setUpdateDate(new Date());
        ApiSender api = new YouZhuInformationForSuZhou();
