 ...
 * 襄阳摩尔龙商务信息咨询有限公司-房抵
 */
@Component("apiSender_20082")
public class MoerlongApi implements ApiSender {

    private static final Logger LOG = LoggerFactory.getLogger(MoerlongApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "8f955d7287734951a2d95c8e65578583";

    private static Long channelId = 57l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            LOG.error("【摩尔龙-房抵/车抵】分发异常：{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【摩尔龙-房抵/车抵】分发异常:"+e.getMessage()));
            return new SendResult(false,"【摩尔龙-房抵/车抵】分发异常："+e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("【摩尔龙-房抵/车抵】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【摩尔龙-房抵/车抵】分发成功："+result));
            return new SendResult(true,"【摩尔龙-房抵/车抵】分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【摩尔龙-房抵/车抵】发送重复："+result));
            return new SendResult(false,"【摩尔龙-房抵/车抵】发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【摩尔龙-房抵/车抵】发送失败："+result));
        return new SendResult(false,"【摩尔龙-房抵/车抵】发送失败："+result);
    }
...
//        po.setChannel("ttt-5937-xx");
//        po.setOccupation(0);
//        po.setCreditCard(0);
//        po.setUpdateDate(new Date());
//        ApiSender api = new MoerlongApi();
