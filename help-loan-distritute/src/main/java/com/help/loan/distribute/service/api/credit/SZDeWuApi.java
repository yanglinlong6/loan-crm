...
/**
 * 深圳市德务商业法律顾问邮箱公司 : 信用卡法律咨询
 *
 */
@Component("apiSender_20069")
public class SZDeWuApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(SZDeWuApi.class);

    private static final Logger LOG = LoggerFactory.getLogger(SZDeWuApi.class);

    private static final String URL = "http://crm.dewufagu.com/crm/import/customer";

    private static String KEY = "237fdb46b59c4600bcaca937acb42a99";

    private static Long channelId = 49l;
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try{
            ...
            log.error("[德务法顾-信用卡逾期]分发异常:{}",e.getMessage(),e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【德务法顾-信用卡逾期】分发未知异常："+e.getMessage()));
            return new SendResult(false,"[德务法顾-信用卡逾期]分发异常:"+e.getMessage());
        }
    }
    ...
    private SendResult sendResult2(UserAptitudePO po, UserDTO select){
        ...
        LOG.info("[德务法顾]客户信息:{}",customerPO.toString());
        ...
        LOG.info("【德务法顾-信用卡逾期】发送结果：{}",result);
        int code = json.getIntValue("code");
        if(200 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),1,"【德务法顾-信用卡逾期】分发成功："+result));
            return new SendResult(true,"【德务法顾-信用卡逾期】分发成功："+result);
        }else if(601 == code){
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),0,"【德务法顾-信用卡逾期】发送重复："+result));
            return new SendResult(false,"【德务法顾-信用卡逾期】发送重复："+result);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(),po.getId(),2,"【德务法顾-信用卡逾期】发送失败："+result));
        return new SendResult(false,"【德务法顾-信用卡逾期】发送失败："+result);
    }
