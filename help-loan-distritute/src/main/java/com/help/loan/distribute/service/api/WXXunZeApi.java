 ...
 * 无锡舜泽
 */
@Component("apiSender_20065")
public class WXXunZeApi implements ApiSender {

    private static final Logger log = LoggerFactory.getLogger(WXXunZeApi.class);
    ...
    private static final String tenantId = "053582";

    private static final String channelCode = "98880942";

    private static final String cluePoolCode = "47345015";
    ...
    public SendResult send(UserAptitudePO po, UserDTO select) {
        try {
            if(StringUtils.isNotBlank(po.getChannel()) && po.getChannel().contains("ttt"))
                return new SendResult(false,"[无锡舜泽]暂不接收头条客户");
            return sendResult(po, select);
        } catch (Exception e) {
            log.error("[无锡舜泽]分发异常:{}", e.getMessage(), e);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "【无锡舜泽】分发未知异常：" + e.getMessage()));
            return new SendResult(false, "[无锡舜泽]分发异常:" + e.getMessage());
        }
    }
    ...
    private SendResult sendResult(UserAptitudePO po, UserDTO select) {
        ...
        log.info("[上海宜寻]撞库结果 : {}", checkResult);
        JSONObject json = JSONUtil.toJSON(checkResult);
        if ("duplicate".equals(json.getString("data"))) {
            log.info("[无锡舜泽]撞库-重复:{}", checkResult);
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "【无锡舜泽】撞库重复：" + checkResult));
            return new SendResult(false, "[无锡舜泽]撞库结果-重复: " + checkResult);
        }
        data.clear();
        data.put("tenantId", tenantId);
//        String channel = po.getChannel();
//        if (StringUtils.isNotBlank(channel) && channel.contains("ttt")) {
//            data.put("channelCode", toutiaoChannelCode);
//        } else
//            data.put("channelCode", WechartChnannelCode);
        ...
        log.info("【无锡舜泽】分发结果：{}", sendResult);
        int code = JSONUtil.toJSON(sendResult).getIntValue("code");
        if (200 == code) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 1, "【无锡舜泽】http表单分发成功：" + sendResult));
            return new SendResult(true, "【无锡舜泽】分发成功：{}" + sendResult);
        } else if (301 == code) {
            dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 0, "【无锡舜泽】分发重复：" + sendResult));
            return new SendResult(false, "【无锡舜泽】分发重复：{}" + sendResult);
        }
        dispatcheRecDao.add(getDispatcheRecPO(po.getOrgId(), po.getId(), 2, "【无锡舜泽】分发失败：" + sendResult));
        return new SendResult(false, "【无锡舜泽】分发失败：{}" + sendResult);
    }
