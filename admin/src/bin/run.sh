...
start(){
    if [ -f $pid_file ];then
        ...
        if [ $a -gt 0 ]; then
            ...
            rm -rf $pid_file
            java $JAVA_OPTS_PRO -jar $work_path/admin-*.jar --spring.profiles.active=pro  >/dev/null 2>&1 &
        fi
    else
        java $JAVA_OPTS_PRO -Dloader.path=$WORKDIR/lib -jar $work_path/admin-*.jar  --spring.profiles.active=pro  >/dev/null 2>&1 &
...
stop(){
  pid=`jps | grep admin | awk '{print $1}'`
  kill $pid
